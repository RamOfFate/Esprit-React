...
import { ArrowLeft, DollarSign, Ticket, User } from "lucide-react";
  ...
  return (
    ...
    <div className="">
      <Button variant="ghost" asChild className="mb-4">
        <Link to="/"><ArrowLeft className="mr-2 h-4 w-4" /> Back to Events</Link>
      </Button>

      <div className="grid grid-cols-2 gap-8">
        <div className="aspect-video object-cover relative">
            <img src={event.img} alt={event.name} className="h-full aspect-video" />
        ...
        <div className="space-y-2">
          ...
          <Button className={`w-full h-12 text-lg ${tickets === 0? "cursor-default": ""}`} onClick={()=>book()} variant={`${tickets === 0? "secondary": "default"}`}>Confirm Booking</Button>
        </div>
      </div>
    </div>
  );
